...
async function main() {
  ...
  let items = [];
  while (next) {
    const { body = {} } = await spotifyApi.getUserPlaylists("flut1", {
      limit,
      offset,
    });
    if (!body.items) {
  ...
  const testTafelPlaylists = items.filter(
    (playlist) => playlist.public && playlist.name.startsWith("TestTafel")
  );
  ...
  const data = await asyncMap(testTafelPlaylists, async (playlist) => {
    ...
    const tracks = details.tracks.items.map((entry) => {
      return {
        ...
        age: differenceInDays(
          new Date(entry.added_at),
          new Date(entry.track.album.release_date)
        ),
      };
    });
    ...
    const unpopularTracksArtists = await asyncMap(
      tracks
        .filter(
          (entry) =>
            ![...allUnpopulairArtists, ...newMusicArtists].some(
              ({ id }) => id === entry.track.artists[0].id
            )
        )
        .slice(0, 10)
    ...
    return {
      ...Object.fromEntries(
        Object.entries(playlist).filter(([key]) =>
          [
            "external_urls",
            "name",
            "tracks",
